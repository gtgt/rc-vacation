...
class vacation extends rcube_plugin
{
	...
	protected function add_date_field($table, $field_id, $val, $format)
	{
		...
		$table->add('title', html::label($field_id, html::quote($this->gettext($field_id))));
		if ($val !== null) {
			if ($val == 0)
			{
				$val = time();
	...
	public function vacation_form()
	{
		...
		$table->add('title', html::label($field_id, html::quote($this->gettext('vacationenable'))));
		...
		if ($this->rc->config->get('vacation_gui_vacationsubject', FALSE))
		{
			...
			$table->add('title', html::label($field_id, html::quote($this->gettext('vacationsubject'))));
		...
		$table->add('title', html::label($field_id, html::quote($this->gettext('vacationmessage'))));
		...
		if ($this->rc->config->get('vacation_gui_vacationkeepcopyininbox', FALSE))
		{
			...
			$table->add('title', html::label($field_id, html::quote($this->gettext('vacationkeepcopyininbox'))));
		...
		if ($this->rc->config->get('vacation_gui_vacationforwarder', FALSE))
		{
			...
			$table->add('title', html::label($field_id, html::quote($this->gettext('vacationforwarder'))));
